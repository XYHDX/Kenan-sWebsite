      ...
      try {
        ...
        if (response.ok) {
          ...
          if (data && 'name' in data && data.name) {
            ...
            console.log('Updated form data from API');
          } else {
            console.log('API returned empty or invalid data, using localStorage fallback');
            // Try fallback to localStorage
            const localData = getFromLocalStorage<ProfileData>(STORAGE_KEYS.PROFILE, formData);
            if (localData && localData.name) {
              setFormData(localData);
              console.log('Updated form data from localStorage');
            }
          }
        } else {
          console.log('API request failed, using localStorage fallback');
          // Try fallback to localStorage
          const localData = getFromLocalStorage<ProfileData>(STORAGE_KEYS.PROFILE, formData);
          if (localData && localData.name) {
            setFormData(localData);
    ...
    <AdminLayout activePage="profile">
      ...
      {isLoading ? (
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
          <form onSubmit={handleSubmit} key={formData.profileImage}>
